...
**Problem Statement:** Our objective is to assign difficulty levels (easy, moderate, or hard) to the MacLeish trails based on length, steepness, and terrain consistency. 

After talking to Professor Bertone Johnson about possible classifications of the trails besides difficulty level, we decided to additionally classify the trails based on biodiversity and proximity to Arts Afield Sites.
...
We computed the lengths of each MacLeish trail, and combined them with the 10’ contour data from MassGIS, since each trail intersects with multiple contours, and at multiple points per contour. According to this [rating system](https://www.nps.gov/shen/planyourvisit/how-to-determine-hiking-difficulty.htm) by National Park Service, all MacLeish trails would be rated "Easy", so we developed our own heuristic. The score for each trail is calculated relative to the most extreme score, as explained in the Length category:
...
* **Elevation:** Rated out of 10.
    - **Steepness:** Rated out of 5; the steepest trail scores 5. The mean change in elevation in a trail.
    - **Terrain consistency:** Rated out of 5; the most uneven trail scores 5. The number of elevation changes in a trail.
...
For this category, we used data from the forests layer. The scores were calculated relative to the most extreme score in each category, similar to our calculations for level of difficulty. Our heuristic to rate the biodiversity of trails is:
...
The most biologically diverse trails are the looping ones, such as the Eastern/Western loops. The least biologically diverse trails are part of the Easy Out trail, which makes sense because they are short trail connectors, and therefore were not built for the purpose of exploring nature, but to help facilitate traffic.
...
MacLeish is building a kiosk for the field station, and will be posting materials that are useful to visitors. Our classifications can be used to build a detailed, inclusive guide for visitors, especially for Smith students looking to further their learning. Different organisations can select trails with a suitable level of difficulty. For example, organisations looking for opportunities to bond can choose an “Easy” trail, while athletic organisations looking to hone their fitness can choose a “Hard” trail. Professors teaching plant biology courses could to take their class to MacLeish, and choose a trail with the most biodiversity, and/or one that crosses the most forests. Additionally, art professors could choose trails that are in proximity to the most arts sites to bring students to, or trails that are near specific arts sites.
A tour guide with these classifications will help visitors make more informed trail choices, optimising the use of their time and gaining a more individualised and meaningful experience. Our classifications are scalable, so one could do similar classifications for other factors once more data about MacLeish are available. For a field station like MacLeish, being able to attract a lot of visitors is key to its sustainability, development, and contribution to academia, which simultaneously benefits Smith.
