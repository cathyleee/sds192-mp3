...
Please see our Github repository[^github] for more details on this project. Most of the data we used are in the `macleish` package retrieved from Macleish Field Station owned by Smith College. 
...
For difficulty level classification, we combined data from `trails` and `contours_3m` layers in `macleish` package. According to this [rating system](https://www.nps.gov/shen/planyourvisit/how-to-determine-hiking-difficulty.htm) by National Park Service, all MacLeish trails would be rated "Easy", so we developed our own heuristic to make rating relative to Macleish situation. The score for each trail is calculated relative to the highest score, as explained in the Length category:
